...
describe("initialSetupSelection", () => {
  it("preselects the active provider card when it is a known preset", () => {
    const selection = initialSetupSelection(
      settingsWithActiveXiaomiWithoutKey(),
    );
  ...
  it("preselects the token plan mode for token plan profiles", () => {
    const current = settings({
      agents: { kun: { providerId: "minimax-token-plan" } },
    });
  ...
  it("preselects the saved permission mode", () => {
    const current = settings({
      agents: {
        kun: { approvalPolicy: "on-request", sandboxMode: "workspace-write" },
      },
    });
    expect(initialSetupSelection(current).permissionMode).toBe(
      "workspace-write",
    );

    const trusted = settings({
      agents: {
        kun: { approvalPolicy: "auto", sandboxMode: "workspace-write" },
      },
    });
    expect(initialSetupSelection(trusted).permissionMode).toBe(
      "trusted-workspace",
    );
  });
});
...
describe("initialSetupDrafts", () => {
  it("seeds drafts from saved profiles and preset defaults", () => {
    ...
    expect(drafts.deepseek).toEqual({
      apiKey: "sk-deepseek-key",
      baseUrl: "https://api.deepseek.com",
    });
    ...
    expect(drafts["minimax-token-plan"].baseUrl).toBe(
...
describe("buildInitialSetupSettings", () => {
  ...
  it("syncs the deepseek draft into the provider profile used by settings", () => {
    ...
    const deepseek = provider.providers.find(
  ...
  it("creates a token plan profile and activates it", () => {
    ...
    const profile = getModelProviderSettings(next).providers.find(
      (p) => p.id === "xiaomi-token-plan",
    );
    ...
    expect(profile?.modelProfiles["mimo-v2.5"]).toEqual(
  ...
  it("auto-wires speech and image to filled pay-as-you-go profiles", () => {
    ...
    expect(
      getModelProviderSettings(next).providers.find((p) => p.id === "xiaomi")
        ?.speech?.protocol,
    ).toBe("mimo-asr");
  });
  ...
  it("wires speech from a xiaomi token plan key and image from a minimax token plan key", () => {
    ...
    tokenPlanOnly["xiaomi-token-plan"] = {
      ...tokenPlanOnly["xiaomi-token-plan"],
      apiKey: "tp-key",
    };
    tokenPlanOnly["minimax-token-plan"] = {
      ...tokenPlanOnly["minimax-token-plan"],
      apiKey: "mm-tp-key",
    };
    expect(initialSetupAutoWirePlan(settings(), tokenPlanOnly)).toEqual({
      speechProviderId: "xiaomi-token-plan",
      imageProviderId: "minimax-token-plan",
    });
  });
  ...
  it("never overrides existing speech or image generation config while auto-wiring", () => {
    const configured = settings({
      agents: { kun: { speechToText: { providerId: "custom" } } },
    });
    const drafts = initialSetupDrafts(configured);
    drafts.xiaomi = { ...drafts.xiaomi, apiKey: "sk-mimo-key" };
    const next = buildInitialSetupSettings(configured, drafts, {
      presetId: "xiaomi",
      mode: "api",
    });
    expect(getKunRuntimeSettings(next).speechToText.providerId).toBe("custom");

    const imageConfigured = settings({
      agents: { kun: { imageGeneration: { providerId: "custom-image" } } },
    });
    const imageDrafts = initialSetupDrafts(imageConfigured);
    imageDrafts["minimax-token-plan"] = {
      ...imageDrafts["minimax-token-plan"],
      apiKey: "mm-tp-key",
    };
    const nextImage = buildInitialSetupSettings(imageConfigured, imageDrafts, {
      presetId: "minimax",
      mode: "token-plan",
    });
    expect(getKunRuntimeSettings(nextImage).imageGeneration.providerId).toBe(
      "custom-image",
    );
  });
  ...
  it("prefers the pay-as-you-go profile for speech when both keys are filled", () => {
    ...
    expect(initialSetupAutoWirePlan(settings(), drafts).speechProviderId).toBe(
      "xiaomi",
    );
  });
  ...
  it("preserves unrelated custom providers", () => {
    ...
    const zenmux = getModelProviderSettings(next).providers.find(
      (p) => p.id === "custom-provider-2",
    );
...
describe("initialSetupProfileId", () => {
  it("maps selection to profile ids", () => {
    expect(initialSetupProfileId({ presetId: "deepseek", mode: "api" })).toBe(
      "deepseek",
    );
    expect(
      initialSetupProfileId({ presetId: "xiaomi", mode: "token-plan" }),
    ).toBe("xiaomi-token-plan");
    expect(initialSetupProfileId({ presetId: "minimax", mode: "api" })).toBe(
      "minimax",
    );
  });
});
