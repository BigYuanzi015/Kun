...
const DEFAULT_WRITE_WORKSPACE_ROOT_ABSOLUTE = expandHomePath(
  DEFAULT_WRITE_WORKSPACE_ROOT,
);
...
const RECOVERABLE_WORKSPACE_ROOT_ERROR_CODES = new Set([
  "ENOENT",
  "ENOTDIR",
  "EACCES",
  "EPERM",
...
function normalizeConversationWorkspaceRoot(
  ...
  const credentialId =
    credential?.kind === "feishu"
      ...
      : credential?.kind === "weixin"
        ...
        : "";
  const workspaceId = sanitizePathSegment(
...
function normalizeClawChannelWorkspaceRoot(channel: ClawImChannelV1): string {
  return (
    expandHomePath(channel.workspaceRoot) ||
    defaultClawChannelWorkspaceRoot(channel)
  );
}

function sanitizeConversationWorkspaceSegment(
...
function defaultClawConversationWorkspaceRoot(
  ...
  return join(
    normalizeClawChannelWorkspaceRoot(channel),
    "conversations",
...
function normalizeClawConversationWorkspaceRoot(
  ...
  return (
    expandHomePath(conversation.workspaceRoot) ||
    defaultClawConversationWorkspaceRoot(channel, conversation)
  );
}
...
function normalizeStoredSettings(settings: AppSettingsV1): AppSettingsV1 {
  ...
  const writeWorkspaces = [
    ...new Set(
      [
        writeDefaultRoot,
        writeActiveRoot,
        ...normalized.write.workspaces.map(normalizeWriteWorkspaceRoot),
      ].filter(Boolean),
    ),
  ];
  return {
    ...
    conversationWorkspaceRoot: normalizeConversationWorkspaceRoot(
      normalized.conversationWorkspaceRoot,
    ),
    write: {
      ...normalized.write,
      defaultWorkspaceRoot: writeDefaultRoot,
      activeWorkspaceRoot: writeWorkspaces.includes(writeActiveRoot)
        ? writeActiveRoot
        : writeDefaultRoot,
      workspaces:
        writeWorkspaces.length > 0 ? writeWorkspaces : [writeDefaultRoot],
    },
    claw: {
      ...normalized.claw,
      channels: normalized.claw.channels.map((channel) => ({
        ...
        conversations: channel.conversations.map((conversation) => ({
          ...conversation,
          workspaceRoot: normalizeClawConversationWorkspaceRoot(
            channel,
            conversation,
          ),
        })),
...
function errnoCode(error: unknown): string {
  return isErrnoException(error) && typeof error.code === "string"
    ? error.code
    : "";
}
...
async function ensureSettingsWorkspaceRoot(
  settings: AppSettingsV1,
): Promise<AppSettingsV1> {
  const workspaceRoot = normalizeWorkspaceRoot(settings.workspaceRoot);
  try {
    await ensureWorkspaceRootExists(workspaceRoot);
    return settings.workspaceRoot === workspaceRoot
      ? settings
    ...
    if (
      ...
      !isRecoverableWorkspaceRootError(error)
    )
      throw error;
...
async function ensureWriteWorkspaceRootsExist(
  settings: AppSettingsV1,
): Promise<void> {
  ...
  try {
    await writeFile(welcomePath, WELCOME_MARKDOWN, {
      encoding: "utf8",
      flag: "wx",
    });
  } catch (error) {
...
async function ensureConversationWorkspaceRootExists(
  settings: AppSettingsV1,
): Promise<void> {
  const root = normalizeConversationWorkspaceRoot(
    settings.conversationWorkspaceRoot,
  );
  if (!root) return;
...
async function prepareSettingsDirectories(
  settings: AppSettingsV1,
): Promise<AppSettingsV1> {
...
function buildMergedSettings(parsed: Partial<AppSettingsV1>): AppSettingsV1 {
  ...
  return {
    ...
    agents: kunSettingsEnvelope(
      mergeKunRuntimeSettings(
        getKunRuntimeSettings(defaults),
        migrated.agents?.kun,
      ),
    ),
    ...
    codePromptPrefix:
      typeof migrated.codePromptPrefix === "string"
        ? migrated.codePromptPrefix
        : "",
    disabledSkillIds: normalizeDisabledSkillIds(migrated.disabledSkillIds),
    installedPluginIds: normalizeInstalledPluginIds(
      migrated.installedPluginIds,
    ),
  };
}
...
async function writeInvalidSettingsBackup(
  path: string,
  raw: string,
): Promise<string | null> {
...
async function writeLegacyCredentialSettingsBackup(
  path: string,
  raw: string,
): Promise<string | null> {
  const backupPath = join(
    dirname(path),
    `${basename(path, ".json")}.pre-extension-credential-migration.json`,
  );
  try {
...
export class JsonSettingsStore {
  ...
  async load(): Promise<AppSettingsV1> {
    ...
    try {
      ...
      throw new Error(
        `Failed to read settings file ${sourcePath}: ${message}`,
        { cause: error },
      );
    }
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      throw new Error(
        `Failed to parse settings file ${sourcePath}: ${message}`,
        { cause: error },
      );
    }
    ...
    const normalized = normalizeStoredSettings(
      buildMergedSettings(parsed as Partial<AppSettingsV1>),
    );
    const prepared = await prepareSettingsDirectories(normalized);
    if (
      this.options.credentialMigration &&
      hasLegacyProviderPlaintext(prepared)
    ) {
      const backupPath = await writeLegacyCredentialSettingsBackup(
        sourcePath,
        raw,
      );
      if (!backupPath) {
        console.warn(
          "[kun-gui] Legacy credential migration deferred because the settings backup could not be written.",
        );
        this.cache = prepared;
        return this.cache;
      }
    ...
    if (shouldPersist) {
      try {
        ...
        console.warn(
          "[kun-gui] Legacy credential migration settings commit failed; plaintext settings remain authoritative.",
          {
            message: error instanceof Error ? error.message : String(error),
          },
        );
    ...
    await migration.commit().catch((error) => {
      console.warn(
        "[kun-gui] Legacy credential migration commit marker is pending recovery.",
        {
          message: error instanceof Error ? error.message : String(error),
        },
      );
  ...
  async save(data: AppSettingsV1): Promise<void> {
    ...
    if (
      ...
      const currentRaw = await readFile(this.path, "utf8").catch((error) => {
        if (isErrnoException(error) && error.code === "ENOENT")
          return serializeSettingsForDisk(prepared);
        throw error;
      });
      const backupPath = await writeLegacyCredentialSettingsBackup(
        this.path,
      ...
      if (!backupPath) {
        throw new Error(
          "Failed to create the pre-migration settings backup; ordinary settings were not changed",
        );
      }
    }
    ...
    if (migration === null)
      throw new Error("Legacy credential migration is unavailable");
    try {
      await this.persistSettings(migration.persistedSettings);
    } catch (error) {
      await migration.rollback().catch(() => undefined);
  ...
  private async prepareCredentialMigration(
    ...
    try {
      ...
      console.warn(
        "[kun-gui] Legacy credential migration is unavailable; retaining compatibility settings.",
        {
          message: error instanceof Error ? error.message : String(error),
        },
      );
