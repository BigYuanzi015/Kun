    ...
    return value.threads
      .flatMap((candidate) => {
        ...
        if (
          ...
          return [];
        const status =
          typeof thread.status === "string" ? thread.status : "idle";
...
export function buildTrayMenuTemplate(input: {
  ...
  appendThreadGroup(
    template,
    labels.running,
    running.slice(0, PRIMARY_GROUP_LIMIT),
    input.actions,
  );
  appendThreadGroup(
    template,
    labels.recent,
    recent.slice(0, PRIMARY_GROUP_LIMIT),
    input.actions,
  );

  const overflow = [
    ...running.slice(PRIMARY_GROUP_LIMIT),
    ...recent.slice(PRIMARY_GROUP_LIMIT),
