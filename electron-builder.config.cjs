...
// feed 根目录直接放 latest.yml 和安装包，不再分 channels/<channel>/latest 子目录。
// 可用环境变量 R2_PUBLIC_BASE_URL / R2_RELEASE_PREFIX 覆盖。
const r2PublicBaseUrl = (
  process.env.R2_PUBLIC_BASE_URL || "https://app.sun-golden.cn"
)
  .trim()
  .replace(/\/+$/, "");
const r2ReleasePrefix = (process.env.R2_RELEASE_PREFIX || "sungolden-agent")
  .trim()
  .replace(/^\/+|\/+$/g, "");
const updateChannel = normalizeUpdateChannel(
  envWithLegacyFallback("KUN_UPDATE_CHANNEL", "DEEPSEEK_GUI_UPDATE_CHANNEL") ||
    "stable",
);
const genericUpdateUrl = `${r2PublicBaseUrl}/${r2ReleasePrefix}/`;
...
const releaseArtifactVersion = (
  envWithLegacyFallback(
    "KUN_ARTIFACT_VERSION",
    "DEEPSEEK_GUI_ARTIFACT_VERSION",
  ) || ""
).trim();
const artifactVersion =
  releaseArtifactVersion || releaseAppVersion || "${version}";
...
function normalizeUpdateChannel(raw) {
  ...
  throw new Error(
    `KUN_UPDATE_CHANNEL (or legacy DEEPSEEK_GUI_UPDATE_CHANNEL) must be "stable" or "frontier", got: ${raw}`,
  );
}
...
if (
  ...
  !artifactVersionPattern.test(releaseArtifactVersion)
) {
  throw new Error(
...
module.exports = {
  ...
  directories: {
    output:
      envWithLegacyFallback("KUN_DIST_DIR", "DEEPSEEK_GUI_DIST_DIR") || "dist",
  },
  files: [
    "out/**/*",
    "package.json",
  ...
  mac: {
    ...
    timestamp: hasExplicitMacSigningIdentity
    ...
    extendInfo: {
      ...
      NSMicrophoneUsageDescription:
        "Kun uses the microphone for voice-to-text input.",
    },
  ...
  nsis: {
    ...
    shortcutName: "SunGolden",
    uninstallDisplayName: "SunGolden",
