...
export class CompatModelClient implements ModelClient {
  ...
  constructor(config: CompatModelClientConfig) {
    ...
    this.fetchImpl =
      config.fetchImpl ?? createProxyFetch(config.modelProxyUrl ?? "") ?? fetch;
  }
  ...
  private async *streamInner(
    ...
    const url = buildModelEndpointUrl(
      this.config.baseUrl,
      configuredEndpointFormat,
    );
    ...
    const modelStreamLimits = normalizeModelStreamLimits(
      this.config.streamLimits,
    );
    const maxErrorBodyBytes = Math.min(
      modelStreamLimits.maxTotalBytes,
      1 * 1024 * 1024,
    );
    ...
    for (let attempt = 0; attempt < retry.maxAttempts; attempt += 1) {
      ...
      if (result.response.ok || !retryStatuses.has(result.response.status))
        break;
    ...
    if (!response.ok) {
      const errorBody = await readLimitedResponseText(
        response,
        maxErrorBodyBytes,
      );
      ...
      if (
        ...
        const retryBody = this.buildRequestBody(request, stream, {
          endpointFormat,
          includeStreamUsage: false,
        });
        if (round)
          this.config.debugSink?.captureRequest(
            round,
            retryBody,
            redactUrlForLog(url),
          );
        ...
        if (response.ok) {
          if (
            ...
            const json = await readLimitedResponseJson(
              response,
              modelStreamLimits.maxTotalBytes,
            );
            if (json.kind === "limit") {
            ...
            if (json.kind === "invalid_json") {
              yield {
                kind: "error",
                message: `model response contained invalid JSON: ${json.message}`,
              };
              return;
            }
        ...
        const retryErrorBody = await readLimitedResponseText(
          response,
          maxErrorBodyBytes,
        );
        ...
        const retryClassified = await this.classifyHttpError(
          response.status,
          retryText,
        );
        yield {
  ...
  private endpointFormat(): ModelEndpointFormat {
    return normalizeModelEndpointFormat(
      this.config.endpointFormat ?? DEFAULT_MODEL_ENDPOINT_FORMAT,
    );
  }
  ...
  private modelReasoningFor(
    model: string,
  ): ModelCapabilityMetadata["reasoning"] | undefined {
  ...
  > {
    try {
      ...
      const proxyHint =
        !aborted && this.config.modelProxyUrl?.trim()
  ...
  private buildRequestBody(
    ...
    const isCodex = this.config.baseUrl.includes(
      "chatgpt.com/backend-api/codex",
    );
    const isCodexLite =
      isCodex && this.capabilitiesForModel(model).responsesMode === "lite";
    ...
    return codecs.build({
      ...
      codexNativeImageGeneration:
        codexModelSupportsNativeImageGeneration(model),
    });
  }
  ...
  private async *streamSse(
    ...
    let bufferBytes = 0;
    const idleTimeoutMs = normalizeStreamIdleTimeoutMs(
    ...
    try {
      while (!signal.aborted) {
        ...
        while (true) {
          ...
          const consumedBytes =
            ...
            Buffer.byteLength(parsedFrame.delimiter, "utf8");
          ...
          try {
            ...
            yield {
              kind: "error",
              message: "model stream contained invalid SSE JSON",
          ...
          if (result.finishReason) {
            ...
            finishReason = mergeStreamFinishReason(
              finishReason,
              result.finishReason,
            );
          }
      ...
      if (!readerFinished)
        cancelReader("model stream closed before body completion");
      try {
        reader.releaseLock();
      } catch {
    ...
    stopReason = ((): ModelStopReason => {
      switch (finishReason) {
        ...
        default:
          ...
          return flushedPendingToolCall || completedToolCalls.size > 0
            ? "tool_calls"
            : "stop";
      }
    })();
...
function modelPayloadError(
  ...
  const responseError = modelErrorObject(
    recordValue(recordValue(payload, "response"), "error"),
  );
  if (responseError) return responseError;
  const baseResp =
    recordValue(payload, "base_resp") ?? recordValue(payload, "baseResp");
  if (baseResp) {
  ...
  const topLevelCode = errorCodeString(
    payload.code ?? payload.type ?? payload.status_code ?? payload.err_code,
  );
...
function modelErrorObject(
  ...
  if (code && !successErrorCode(code))
    return { message: `model provider error (${code})`, code };
  return null;
}
...
function mergeUsageSnapshots(
  ...
  const totalTokens =
    next.totalTokens > 0 && next.promptTokens > 0
      ? next.totalTokens
...
function normalizeModelStreamLimits(
  input: Partial<ModelStreamLimits> | undefined,
): ModelStreamLimits {
  ...
  return {
    maxBufferBytes: normalize(
      input?.maxBufferBytes,
      DEFAULT_MODEL_STREAM_LIMITS.maxBufferBytes,
    ),
    maxFrameBytes: normalize(
      input?.maxFrameBytes,
      DEFAULT_MODEL_STREAM_LIMITS.maxFrameBytes,
    ),
    maxTotalBytes: normalize(
      input?.maxTotalBytes,
      DEFAULT_MODEL_STREAM_LIMITS.maxTotalBytes,
    ),
    maxFrames: normalize(
      input?.maxFrames,
      DEFAULT_MODEL_STREAM_LIMITS.maxFrames,
    ),
    maxOutputBytes: normalize(
      input?.maxOutputBytes,
      DEFAULT_MODEL_STREAM_LIMITS.maxOutputBytes,
    ),
    maxPendingToolCalls: normalize(
      input?.maxPendingToolCalls,
      DEFAULT_MODEL_STREAM_LIMITS.maxPendingToolCalls,
    ),
    ...
    maxCompletedToolCalls: normalize(
      input?.maxCompletedToolCalls,
      DEFAULT_MODEL_STREAM_LIMITS.maxCompletedToolCalls,
    ),
...
async function readLimitedResponseText(
  ...
  if (Number.isFinite(declaredLength) && declaredLength > maxBytes) {
    void response.body
      ?.cancel("model response body limit exceeded")
      .catch(() => {});
...
async function readStreamChunk(
  ...
  const candidates: Array<Promise<StreamReadResult>> = [
    readPromise,
    abortPromise,
  ];
  if (idleTimeoutMs > 0) {
